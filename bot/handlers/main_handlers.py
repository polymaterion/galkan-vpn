...
async def _render_entry(
    ...
    if screen == "main":
        ...
        text = t("instructions_text", lang)
        markup = instructions_keyboard(
            lang, settings.AMNEZIA_DOWNLOAD_LINK, settings.AMNEZIA_INSTRUCTION_LINK
        )
...
async def cb_instructions(cb: CallbackQuery, state: FSMContext, lang: str):
    await navigation.push(state, "instructions")
    await _render_entry(cb.message, state, lang, await navigation.current(state))
    await cb.answer()
