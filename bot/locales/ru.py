...
STRINGS: dict[str, str] = {
    ...
    "bc_start": (
        "📣 <b>Рассылка</b>\n\n"
        "Пришлите сообщение для рассылки: текст, фото/видео с подписью, "
        "или GIF. Можно использовать HTML-разметку (<b>жирный</b>, <i>курсив</i>).\n\n"
        "Для отмены — /cancel"
    ),
    "bc_empty_content": "Сообщение не может быть пустым. Пришлите текст, фото или видео с подписью.",
    "bc_ask_buttons": (
        "Теперь можно добавить кнопки-ссылки под сообщением (необязательно).\n\n"
        "Формат — одна кнопка на строку:\n"
        "<code>Текст кнопки - https://example.com</code>\n\n"
        "Максимум 2 кнопки. Если кнопки не нужны — отправьте «-» или «Пропустить».\n\n"
        "Для отмены — /cancel"
    ),
    "bc_btn_skip": "Пропустить",
    "bc_invalid_button_line": (
        "Не удалось разобрать строку {n}: «{line}».\n"
        "Формат: <code>Текст кнопки - https://example.com</code>. Попробуйте ещё раз."
    ),
    "bc_invalid_button_url": (
        "Ссылка в строке {n} должна начинаться с http:// или https://: «{url}»."
    ),
    "bc_too_many_buttons": "Слишком много кнопок ({n}). Максимум 2 — пришлите заново.",
    "bc_preview_title": "👁 <b>Предпросмотр рассылки</b>\n\nТак сообщение увидят пользователи:",
    "bc_confirm_prompt": "Получателей: <b>{count}</b>. Отправить рассылку?",
    "btn_bc_send": "🚀 Отправить",
    "btn_bc_cancel": "❌ Отмена",
    "bc_cancelled": "Рассылка отменена.",
    "bc_sending": "⏳ Рассылка запущена: {total} получателей. Это может занять некоторое время...",
    "bc_done": (
    ...
    "bc_no_recipients": "Нет получателей для рассылки (в базе нет пользователей).",

    # --- Trial ---
    "btn_trial": "🎁 Попробовать бесплатно 3 дня",
    "trial_confirm_text": (
        "🎁 <b>Пробный период — 3 дня бесплатно</b>\n\n"
        "Полный доступ к VPN на 3 дня, без оплаты. Активируется один раз.\n\n"
        "Активировать?"
    ),
    "btn_trial_activate": "🎁 Получить 3 дня бесплатно",
    "trial_granting": "⏳ Активируем пробный период...",
    "trial_already_used": "Вы уже использовали пробный период. Оформите платную подписку в разделе «Мои устройства».",
    "trial_unavailable": "Пробный период сейчас недоступен. Попробуйте позже или обратитесь в поддержку.",
    "trial_error": "❗ Не удалось активировать пробный период. Попробуйте позже или обратитесь в поддержку.",
}
